...
function isUserEntry(entry: SessionMessageEntry): entry is UserEntry {
...
export default function mdelloCompanion(pi: ExtensionAPI): void {
  ...
  let currentStatus: AssociationStatus = 'idle';

  async function updateStatus(status: AssociationStatus, ctx: ExtensionContext): Promise<void> {
