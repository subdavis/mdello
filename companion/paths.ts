...
function modificationCall(value: unknown, cwd?: string): { id: string; path: string } | undefined {
  ...
  const path = record(call.arguments)?.path;
  if (typeof path !== 'string' || !path.toLowerCase().endsWith('.md')) return undefined;
  if (!path.startsWith('/') && !cwd) return undefined;
  return { id: call.id, path: path.startsWith('/') ? path : resolve(cwd as string, path) };
}
